package jp.cellfusion.video {
	...
	public class FMSVideoPlayer extends Sprite implements IVideoPlayer
	{
		...
		public function FMSVideoPlayer(id : String, width : Number, height : Number)
		{
			url_re = /^rtmp\:\/\/([\S^\/]+?)\/([\S^\/]+?)\/(.+)$/;
		...
		public function play(request : URLRequest = null, play : Boolean = false, start : Number = -2) : void
		{
			...
			if (_metadata && _metadata.duration) {
				...
				addEventListener(VideoEvent.METADATA_RECEIVED, playMetadataStay);
			}
		}
